  ...
  reducers: {
    ...
    ascSort: (state, action) => {
      state.usersList = state.usersList.sort((a, b) => {
        return a[action.payload][0] > b[action.payload][0]
          ? 1
          : a[action.payload][0] === b[action.payload][0]
          ? 0
          : -1;
      });
    },
    ...
    openBox: (state, action) => {
      ...
      if (action.payload.userData && action.payload.typeAction === "Submit") {
        ...
        action.payload.typeAction === "Update"
      ) {
        ...
        state.usersList[idx].plan = [
          action.payload.userData.plan,
          "Select Plan",
        ];
        state.usersList[idx].role = [
          action.payload.userData.role,
          "Select Role",
        ];
        if (
          JSON.parse(localStorage.getItem(KEY)).id === state.usersList[idx].id
        ) {
    ...
    setAddRecordPopup: (state) => {
      state.addRecordPopup = !state.addRecordPopup;
    },
  },
});
...
export const getAddRecordPopup = (state) => state.users.addRecordPopup;
export const profile = (state) => state.users.info;
export const {
  makeProfile,
  userDelete,
  ascSort,
